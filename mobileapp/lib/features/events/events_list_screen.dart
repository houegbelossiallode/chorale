...
class _EventsListScreenState extends State<EventsListScreen> {
  ...
  void _loadEvents() {
    setState(() {
      _eventsFuture = _eventService.fetchEvents().then((events) {
        setState(() {
          _allEvents = events;
        });
        return events;
          ...
          SliverPadding(
            ...
            sliver: FutureBuilder<List<Event>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting && _allEvents == null) {
                  return const SliverFillRemaining(child: Center(child: CircularProgressIndicator()));
                } else if (snapshot.hasError && _allEvents == null) {
                  return SliverFillRemaining(child: Center(child: Text("Erreur: ${snapshot.error}")));
                } else if ((!snapshot.hasData || snapshot.data!.isEmpty) && _allEvents == null) {
                  return SliverFillRemaining(child: Center(child: Text("Aucun événement prévu.")));
                }

                final events = _allEvents ?? snapshot.data!;
  ...
  Widget _buildPresenceChip(Event event, String label, String value, Color color) {
    ...
    return GestureDetector(
      onTap: () async {
        ...
        final String? oldChoice = event.userChoice;
