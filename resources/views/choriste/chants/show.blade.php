...
@section('content')
<div class="w-full min-h-screen bg-slate-50/50 -mt-6 -mx-6 px-6 pt-6">
    {{-- Header Premium --}}
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-4">
                    <h1 class="text-4xl font-extrabold text-[#444050] tracking-tight">{{ $chant->title }}</h1>
                    @if($chant->composer)
                        <span class="px-3 py-1 bg-white border border-[#7367F0]/20 text-[#7367F0] text-xs font-bold rounded-full shadow-sm">
                            {{ $chant->composer }}
                        </span>
                    @endif
                </div>
            ...
            <div class="flex items-center gap-2">
                <div class="px-4 py-2 bg-white rounded-xl shadow-material border border-gray-100 flex items-center gap-3">
                    <div class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                    <span class="text-xs font-bold text-slate-500 uppercase tracking-tight">Mode Apprentissage Actif</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            <div class="bg-white rounded-3xl shadow-material-lg overflow-hidden border border-gray-100" x-data="{ activeTab: '{{ $partitions->first()->id }}' }">
                {{-- Tab Header --}}
                <div class="px-8 py-5 border-b border-gray-100 bg-slate-50/50 flex flex-wrap items-center justify-between gap-4">
                    <div class="flex items-center gap-2 overflow-x-auto custom-scrollbar-slim pb-2 md:pb-0">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <a x-show="activeTab === '{{ $p->id }}'" href="{{ $p->file_path }}" target="_blank" 
                            class="p-2.5 bg-white border border-gray-200 text-red-500 rounded-xl hover:bg-red-50 transition-all shadow-sm" title="Ouvrir plein écran">
                ...
                <div class="p-2 bg-slate-100">
                    @foreach($partitions as $p)
                    <div x-show="activeTab === '{{ $p->id }}'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="aspect-[1/1.4] w-full rounded-2xl overflow-hidden shadow-inner">
                        <iframe src="{{ $p->file_path }}#toolbar=0" class="w-full h-full" frameborder="0"></iframe>
                    </div>
                    @endforeach
                </div>
            </div>
            @else
            <div class="bg-white rounded-3xl p-20 text-center shadow-material border border-dashed border-gray-300">
        ...
        <div class="lg:col-span-4 space-y-6 lg:sticky lg:top-24 self-start">
            ...
            <div class="backdrop-blur-md bg-white/60 border border-white/50 rounded-3xl p-6 shadow-material-lg">
                ...
                <div class="space-y-6" id="recorder-container">
                    <div class="flex items-center justify-center py-4 bg-slate-50/50 rounded-2xl relative overflow-hidden group">
                        ...
                        <button id="startBtn" class="relative z-10 w-20 h-20 bg-red-500 text-white rounded-full flex items-center justify-center shadow-xl hover:bg-red-600 transition-all hover:scale-110 group active:scale-95">
                            <div class="w-6 h-6 bg-white rounded-full"></div>
                        </button>
                        <button id="stopBtn" class="relative z-10 w-20 h-20 bg-slate-900 text-white rounded-full flex items-center justify-center shadow-xl hover:bg-black transition-all hover:scale-110 active:scale-95 hidden">
                    ...
                    <div id="recorderStatus" class="text-center">
                        ...
                        <p id="statusLabel" class="text-xs font-bold text-slate-600 mb-2 uppercase">Prêt à enregistrer</p>
            ...
            <div class="bg-white rounded-3xl shadow-material-lg p-6 border border-gray-100">
                ...
                <div class="space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar-slim pr-1">
                    ...
                    <div class="p-4 rounded-2xl border border-slate-50 bg-slate-50/30 hover:bg-slate-50 transition-colors">
                        ...
                        <div class="p-3 bg-white border border-[#7367F0]/20 rounded-xl relative mt-2 shadow-sm">
                            <div class="absolute -top-2 left-3 px-1.5 bg-[#7367F0] text-white text-[8px] font-black rounded uppercase">Feedback Chef</div>
                            <p class="text-[11px] text-slate-600 font-medium italic leading-relaxed">{{ $rec->chef_comment }}</p>
                        </div>
                        @endif
                    </div>
                    @endforeach
            ...
            <div class="bg-white rounded-3xl shadow-material-lg p-6 border border-gray-100">
                ...
                <div class="mb-5 last:mb-0 p-4 bg-slate-50/50 rounded-2xl group transition-all hover:bg-blue-50/30">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <a href="{{ $audio->file_path }}" download class="w-6 h-6 flex items-center justify-center bg-white rounded-lg border border-gray-100 text-[#7367F0] hover:scale-110 transition-transform shadow-sm">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                        </a>
                    </div>
                    <audio controls class="w-full h-8 custom-audio"></audio>
                </div>
                @endforeach
            </div>
            @endif
            ...
            <div class="bg-white rounded-3xl shadow-material-lg p-6 border border-gray-100">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ $visual->file_path }}" target="_blank" class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50/50 hover:bg-purple-50/30 border border-transparent hover:border-purple-200 transition-all group">
                        @if($visual->type === 'youtube')
                        <div class="w-10 h-10 bg-red-50 text-red-500 rounded-xl flex items-center justify-center shrink-0 shadow-sm group-hover:scale-110 transition-transform">
                        ...
                        @else
                        <div class="w-10 h-10 bg-purple-50 text-purple-500 rounded-xl flex items-center justify-center shrink-0 shadow-sm group-hover:scale-110 transition-transform">
                        ...
                        <div class="overflow-hidden">
                            <p class="text-sm font-bold text-slate-700 truncate group-hover:text-purple-600 transition-colors">{{ $visual->pupitre ? $visual->pupitre->name : 'Général' }}</p>
                            <p class="text-[10px] text-slate-400 font-black uppercase">{{ $visual->type === 'youtube' ? 'YouTube link' : 'Vidéo locale' }}</p>
                        </div>
                    </a>
                    @endforeach
                </div>
