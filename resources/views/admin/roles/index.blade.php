    ...
    <div class="space-y-6" x-data="{ 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($roles as $role)
                <div
                    ...
                    <p class="text-xs text-[#7367F0] font-medium uppercase tracking-wider mb-4">{{ $role->permissions_count }}
                        Permissions accordées</p>

                    <div class="flex flex-col gap-2 pt-4 border-t border-slate-50">
                        <div class="flex items-center justify-between">
                            <button @click="openEditModal({{ $role->id }}, '{{ $role->libelle }}')"
                            ...
                            <a href="{{ route('admin.roles.edit', $role->id) }}"
                                class="text-[14px] font-bold text-[#7367F0] hover:bg-[#7367F0]/10 px-3 py-1.5 rounded-lg transition-all">Permissions</a>
                        </div>
                        <form action="{{ route('admin.roles.destroy', $role->id) }}" method="POST"
                            onsubmit="return confirm('Supprimer ce rôle ?');" class="mt-2 flex justify-end">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
            ...
            <button @click="openCreateModal()"
                class="bg-gray-50 border-2 border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center gap-3 hover:bg-[#7367F0]/5 hover:border-[#7367F0]/30 transition-all group">
                <div
        ...
        <div x-show="roleModalOpen"
            ...
            <div class="bg-white rounded-2xl shadow-material-lg w-full max-w-md overflow-hidden"
                @click.away="roleModalOpen = false" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100">
                <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                    <h3 class="text-xl font-bold text-[#444050]" x-text="editMode ? 'Modifier le rôle' : 'Nouveau rôle'">
                    </h3>
                ...
                <form :action="actionUrl" method="POST" class="p-8 space-y-6">
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        ...
                        <button type="submit" class="btn-primary min-w-[120px]"
                            x-text="editMode ? 'Enregistrer' : 'Créer le rôle'"></button>
                    </div>
                </form>
            </div>
        </div>
