    ...
    <div class="flex-1 overflow-y-auto px-4 py-2 custom-scrollbar">
        <nav class="space-y-1 pb-10" x-data="{ activeMenu: '{{ $initialActiveMenu }}' }">
            ...
            <a href="{{ route('admin.dashboard') }}"
                ...
                <span class="text-[15px] font-medium">Tableau de bord</span>
            </a>

            <!-- <a href="{{ route('admin.audit-logs.index') }}"
                class="flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all group {{ request()->routeIs('admin.audit-logs.index') ? 'nav-pill-active' : 'text-[#444050] hover:bg-[#F2F2F2]' }}">
                <svg class="w-5 h-5 {{ request()->routeIs('admin.audit-logs.index') ? 'text-white' : 'text-slate-400 group-hover:text-[#7367F0]' }}"
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <span class="text-[15px] font-medium">Piste d'Audit</span>
            </a> -->
            @foreach($mainmenus as $menu)
                @php
