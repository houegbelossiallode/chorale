                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-slate-100">
                            <th class="px-8 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Libellé
                            </th>
                            <th class="px-8 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest">URL</th>
                            <th class="px-8 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Menu Parent
                            </th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($sousMenus as $sm)
                            <tr class="hover:bg-slate-50/50 transition duration-300">
                                ...
                                <td class="px-8 py-4">
                                    <code
                                        class="text-[11px] bg-slate-100 px-2 py-1 rounded-md text-slate-600 font-medium">{{ $sm->url }}</code>
                                </td>
                                <td class="px-8 py-4">
                                    <span
                                ...
                                <td class="px-8 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        <button
                                            @click="openSM({{ $sm->id }}, '{{ $sm->name }}', '{{ $sm->url }}', '{{ $sm->menu_id }}')"
                                            class="btn-icon">
                                        ...
                                        <form action="{{ route('admin.sousmenus.destroy', $sm->id) }}" method="POST"
                                            onsubmit="return confirm('Supprimer ce sous-menu ?');">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-8 py-12 text-center text-slate-400 italic font-medium">Aucun sous-menu
                                    configuré.</td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-material-lg"
                ...
                <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-slate-50/50">
                    <h3 class="font-bold text-lg text-[#444050]"
                        x-text="editMode ? 'Modifier le Sous-menu' : 'Nouveau Sous-menu'"></h3>
                ...
                <form :action="actionUrl" method="POST" class="p-8 space-y-6">
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[12px] font-bold uppercase tracking-widest text-slate-500 ml-1">Menu
                            Parent</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[12px] font-bold uppercase tracking-widest text-slate-500 ml-1">URL
                            Relative</label>
