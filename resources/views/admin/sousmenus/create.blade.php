...
@section('title', 'Nouveau Sous-menu')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-3 mb-2">
        <a href="{{ route('admin.sousmenus.index') }}" class="w-9 h-9 bg-white rounded-lg flex items-center justify-center text-slate-400 hover:text-[#7367F0] shadow-sm border border-slate-100 transition-all">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-[#444050]">Créer un Sous-menu</h1>
            <p class="text-slate-500 text-sm">Ajoutez un lien de navigation terminal par menu.</p>
        </div>
    </div>

    <form action="{{ route('admin.sousmenus.store') }}" method="POST" class="bg-white rounded-2xl shadow-material border border-slate-100 overflow-hidden">
        @csrf
        ...
        <div class="p-8 bg-slate-50/50 border-t border-slate-100 flex justify-end gap-3">
            <a href="{{ route('admin.sousmenus.index') }}" class="btn-secondary px-8">Annuler</a>
            <button type="submit" class="btn-primary min-w-[160px]">Créer le Sous-menu</button>
        </div>
    </form>
</div>
@endsection
