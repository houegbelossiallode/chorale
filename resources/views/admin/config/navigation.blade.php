...
@section('content')
    <div class="space-y-6" x-data="{
            ...
            openModule(id = null, name = '') {
                ...
                this.actionUrl = id ? `/admin/modules/${id}` : '{{ route('admin.modules.store') }}';
            ...
            openSM(menuId, id = null, name = '', url = '') {
                ...
                this.actionUrl = id ? `/admin/sousmenus/${id}` : '{{ route('admin.sousmenus.store') }}';
                this.smModal = true;
            }
        }">
                ...
                <div class="bg-white rounded-xl shadow-material overflow-hidden border-l-4 border-[#7367F0]">
                    ...
                    <div class="p-4 bg-slate-50 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <button @click="openMenu({{ $module->id }})"
                                class="text-xs font-bold text-[#7367F0] hover:underline">+ Ajouter Menu</button>
                            <div class="w-px h-4 bg-gray-200 mx-2"></div>
                            <button @click="openModule({{ $module->id }}, '{{ $module->name }}')" class="btn-icon"><svg
                                    class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg></button>
                            <form action="{{ route('admin.modules.destroy', $module->id) }}" method="POST"
                                onsubmit="return confirm('Attention: Supprimer un module supprimera tous ses menus associés.');">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn-icon btn-icon-danger"><svg class="w-5 h-5" fill="none"
                            ...
                            <div class="border border-slate-100 rounded-xl p-4 bg-white hover:shadow-md transition-shadow">
                                <div class="flex items-center justify-between mb-4 pb-2 border-b border-gray-50">
                                    ...
                                    <div class="flex gap-1">
                                        <button @click="openMenu({{ $module->id }}, {{ $menu->id }}, '{{ $menu->name }}')"
                                            class="text-slate-300 hover:text-[#7367F0]"><svg class="w-4 h-4" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <form action="{{ route('admin.menus.destroy', $menu->id) }}" method="POST"
                                            ...
                                            @csrf @method('DELETE')
                                            <button type="submit" class="text-slate-300 hover:text-red-500"><svg class="w-4 h-4"
                                ...
                                <div class="space-y-2">
                                    @foreach($menu->sousMenus as $sm)
                                        <div class="flex items-center justify-between group">
                                            ...
                                            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button
                                                    @click="openSM({{ $menu->id }}, {{ $sm->id }}, '{{ $sm->name }}', '{{ $sm->url }}')"
                                                    class="text-slate-300 hover:text-[#7367F0]"><svg class="w-3 h-3" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                            d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                                    </svg></button>
                                    ...
                                    @endforeach
                                    <button @click="openSM({{ $menu->id }})"
                                        class="text-[11px] font-bold text-slate-400 hover:text-[#7367F0] uppercase tracking-wider mt-2 flex items-center gap-1">
        ...
        <!-- Module Modal -->
        <div x-show="moduleModal"
            class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm" x-cloak>
            <div class="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-material-lg"
                @click.away="moduleModal = false">
                <div class="p-6 border-b border-gray-100 flex justify-between">
                    ...
                    <button @click="moduleModal = false" class="text-slate-400"><svg class="w-6 h-6" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <form :action="actionUrl" method="POST" class="p-8 space-y-4">
                    ...
                    <div>
                        <label class="text-xs font-bold uppercase tracking-widest text-slate-500 block mb-2">Nom du
                            Module</label>
                        <input type="text" name="name" x-model="formData.name"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-200 outline-none focus:border-[#7367F0]"
                            required>
                    </div>
        ...
        <!-- Menu Modal -->
        <div x-show="menuModal"
            class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm" x-cloak>
            <div class="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-material-lg"
                @click.away="menuModal = false">
                <div class="p-6 border-b border-gray-100 flex justify-between">
                    <h3 class="font-bold text-lg" x-text="editMode ? 'Modifier Menu' : 'Nouveau Menu'"></h3>
                    <button @click="menuModal = false" class="text-slate-400"><svg class="w-6 h-6" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                        </svg></button>
                ...
                <form :action="actionUrl" method="POST" class="p-8 space-y-4">
                    ...
                    <div>
                        <label class="text-xs font-bold uppercase tracking-widest text-slate-500 block mb-2">Libellé du
                            Menu</label>
                        <input type="text" name="name" x-model="formData.name"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-200 outline-none focus:border-[#7367F0]"
                            required>
        ...
        <div x-show="smModal"
            class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm" x-cloak>
            <div class="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-material-lg"
                @click.away="smModal = false">
                <div class="p-6 border-b border-gray-100 flex justify-between">
                    <h3 class="font-bold text-lg" x-text="editMode ? 'Modifier Sous-menu' : 'Nouveau Sous-menu'"></h3>
                    <button @click="smModal = false" class="text-slate-400"><svg class="w-6 h-6" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                        </svg></button>
                ...
                <form :action="actionUrl" method="POST" class="p-8 space-y-4">
                    ...
                    <div>
                        <label class="text-xs font-bold uppercase tracking-widest text-slate-500 block mb-2">URL
                            relative</label>
                        <input type="text" name="url" x-model="formData.url"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-200 outline-none focus:border-[#7367F0]"
