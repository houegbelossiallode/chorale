        ...
        <form action="{{ route('admin.donations.store') }}" method="POST">
            ...
            <div class="bg-white rounded-xl shadow-material overflow-hidden">
                <div class="p-8 space-y-6">
                    ...
                    <div x-data="{ newDonor: false }">
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <button type="button" @click="newDonor = !newDonor"
                                class="text-xs text-[#7367F0] font-bold hover:underline">
                        ...
                        <div x-show="newDonor" class="space-y-4">
                            <input type="text" name="donateur_name"
                                class="w-full px-4 py-3 rounded-lg border border-slate-200 focus:border-[#7367F0] outline-none"
                                placeholder="Nom complet du donateur">
                            <input type="email" name="donateur_email"
                                class="w-full px-4 py-3 rounded-lg border border-slate-200 focus:border-[#7367F0] outline-none"
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label
                                class="block text-sm font-bold text-slate-700 mb-3 uppercase tracking-widest">Projet</label>
                    ...
                    <!-- Payment Method -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-3 uppercase tracking-widest">Mode de
                            Paiement</label>
                        <div class="grid grid-cols-3 gap-3">
                            @foreach(['Espèces', 'Mobile Money', 'Virement'] as $method)
                                <label
                                    class="flex items-center justify-center p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-all">
                                    <input type="radio" name="payment_method" value="{{ $method }}" class="hidden peer" {{ $loop->first ? 'checked' : '' }}>
                                    <span
                                        class="text-xs font-bold text-slate-500 peer-checked:text-[#7367F0]">{{ $method }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="p-8 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                    <a href="{{ route('admin.donations.index') }}"
                        class="px-6 py-2.5 rounded-lg text-slate-600 font-medium hover:bg-slate-200 transition-all">Annuler</a>
