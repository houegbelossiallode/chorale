...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <button onclick="window.location.href='{{ route('admin.donations.create') }}'"
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-8 h-8 bg-slate-100 rounded-full flex items-center justify-center text-slate-500 font-bold text-xs">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-2 py-1 bg-indigo-50 text-[#7367F0] rounded text-[10px] font-bold uppercase tracking-wider">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <form action="{{ route('admin.donations.destroy', $donation->id) }}" method="POST"
                                        class="inline"
                                        onsubmit="return confirm('Annuler ce don ? Le montant sera déduit du projet.');">
                                        @csrf
                                        @method('DELETE')
