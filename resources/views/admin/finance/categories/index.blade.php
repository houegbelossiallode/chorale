...
<div class="space-y-6" x-data="{ 
    ...
    catLibelle: '',
    catType: 'recette',
    actionUrl: '{{ route('admin.finance.categories.store') }}',
    ...
    openCreateModal() {
        ...
        this.actionUrl = '{{ route('admin.finance.categories.store') }}';
    ...
    openEditModal(id, libelle, type) {
        ...
        this.catType = type;
        this.actionUrl = `/dashboard/finance-categories/${id}`;
        this.catModalOpen = true;
    }
}">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-[#444050]">Catégories Financières</h1>
            <p class="text-slate-500 text-sm">Définissez les types de revenus et de dépenses pour votre comptabilité.</p>
        </div>
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        @if($category->type == 'recette')
                            <span class="px-2 py-1 bg-green-50 text-green-600 rounded-full text-[10px] font-bold uppercase tracking-wider">Recette</span>
                        @else
                            <span class="px-2 py-1 bg-red-50 text-red-600 rounded-full text-[10px] font-bold uppercase tracking-wider">Dépense</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            <button @click="openEditModal({{ $category->id }}, '{{ $category->libelle }}', '{{ $category->type }}')" class="btn-icon">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </button>
                            <form action="{{ route('admin.finance.categories.destroy', $category->id) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer cette catégorie ?');">
                                @csrf
                                @method('DELETE')
    ...
    <div x-show="catModalOpen" 
        ...
        <div class="bg-white rounded-2xl shadow-material-lg w-full max-w-md overflow-hidden" 
             @click.away="catModalOpen = false"
             x-transition:enter="transition ease-out duration-300"
             x-transition:enter-start="opacity-0 scale-95"
             x-transition:enter-end="opacity-100 scale-100">
            <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                <h3 class="text-xl font-bold text-[#444050]" x-text="editMode ? 'Modifier la catégorie' : 'Nouvelle catégorie'"></h3>
            ...
            <form :action="actionUrl" method="POST" class="p-8 space-y-6">
                ...
                <div>
                    ...
                    <select name="type" x-model="catType"
                            class="w-full px-4 py-3 rounded-lg border border-slate-200 focus:border-[#7367F0] focus:ring-2 focus:ring-[#7367F0]/20 outline-none transition-all" required>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    ...
                    <button type="submit" class="btn-primary min-w-[120px]" x-text="editMode ? 'Enregistrer' : 'Créer'"></button>
                </div>
            </form>
        </div>
    </div>
