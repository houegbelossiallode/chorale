                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            {{-- Bouton de suppression en premier (cohérent avec show) --}}
                            ...
                            @php
                                $isMedia = in_array($fichier->type, ['audio', 'youtube', 'video']);
                                $mediaUrl = $fichier->type === 'youtube' ? $fichier->file_path : (Str::startsWith($fichier->file_path, ['http://', 'https://']) ? $fichier->file_path : asset('storage/' . $fichier->file_path));
                            @endphp

                            @if($isMedia)
                                <button @click="$dispatch('open-media', { type: '{{ $fichier->type }}', url: '{{ $mediaUrl }}', title: '{{ addslashes($chant->title) }}' })"
                                    class="p-2 text-slate-400 hover:text-[#7367F0] transition-colors" title="Voir">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </button>
                            @else
                                <a href="{{ $fichier->file_path }}" download class="p-2 text-slate-400 hover:text-[#7367F0] transition-colors" title="Voir">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                            @endif
                        </div>
