    ...
    <div class="space-y-6" x-data="{ 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            @forelse($pupitres as $pupitre)
                <div
                    class="bg-white rounded-2xl shadow-material border border-slate-100 p-6 hover:shadow-material-lg transition-all duration-300 group">
                    <div class="flex justify-between items-start mb-4">
                        ...
                        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <form action="{{ route('admin.pupitres.destroy', $pupitre) }}" method="POST"
                                onsubmit="return confirm('Supprimer ce pupitre ?');">
                                @csrf @method('DELETE')
                    ...
                    <p class="text-xs text-slate-400 font-medium mb-4 line-clamp-2 min-h-[2rem]">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            @if($pupitre->responsable)
                                <div class="flex items-center gap-2">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($pupitre->responsable->first_name) }}&background=f8f7fa&color=7367f0&size=32"
        ...
        <div x-show="showCreateModal"
            ...
            <div class="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-material-lg animate-fade-in"
                @click.away="showCreateModal = false">
                <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                    <h3 class="font-bold text-lg text-[#444050]">Nouveau Pupitre</h3>
                    <button @click="showCreateModal = false" class="text-slate-400 hover:text-[#7367F0]"><svg
                ...
                <form action="{{ route('admin.pupitres.store') }}" method="POST" class="p-8 space-y-6">
                    @csrf
                    <div class="space-y-2">
                        <label class="text-[12px] font-black text-slate-400 uppercase tracking-widest ml-1">Nom du
                            Pupitre</label>
                        <input type="text" name="name" required
                            class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#7367F0] outline-none transition-all font-bold text-[#444050]">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[12px] font-black text-slate-400 uppercase tracking-widest ml-1">Chef de
                            Pupitre</label>
                    ...
                    <div class="space-y-2">
                        <label
                            class="text-[12px] font-black text-slate-400 uppercase tracking-widest ml-1">Description</label>
                        <textarea name="description" rows="3"
                            class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#7367F0] outline-none transition-all font-medium text-[#444050] resize-none"></textarea>
                    </div>
        ...
        <div x-show="showEditModal"
            ...
            <div class="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-material-lg animate-fade-in"
                ...
                <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                    ...
                    <button @click="showEditModal = false" class="text-slate-400 hover:text-[#7367F0]"><svg class="w-6 h-6"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                        </svg></button>
                </div>
                <form :action="`/admin/pupitres/${currentPupitre.id}`" method="POST" class="p-8 space-y-6">
                    @csrf @method('PUT')
                    <div class="space-y-2">
                        <label class="text-[12px] font-black text-slate-400 uppercase tracking-widest ml-1">Nom du
                            Pupitre</label>
                        <input type="text" name="name" x-model="currentPupitre.name" required
                            class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#7367F0] outline-none transition-all font-bold text-[#444050]">
                    </div>
                    <div class="space-y-2">
                        <label class="text-[12px] font-black text-slate-400 uppercase tracking-widest ml-1">Chef de
                            Pupitre</label>
                        <select name="responsable_id" x-model="currentPupitre.responsable_id"
                            class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#7367F0] outline-none transition-all font-bold text-[#444050]">
                            <option value="">Sélectionner un responsable</option>
                            @foreach(App\Models\User::all() as $user)
                                <option value="{{ $user->id }}">{{ $user->first_name }} {{ $user->last_name }}</option>
                    ...
                    <div class="space-y-2">
                        <label
                            class="text-[12px] font-black text-slate-400 uppercase tracking-widest ml-1">Description</label>
                        <textarea name="description" rows="3" x-model="currentPupitre.description"
                            class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#7367F0] outline-none transition-all font-medium text-[#444050] resize-none"></textarea>
                    </div>
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-50">
