    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-xl sm:text-2xl font-semibold text-[#444050]">Modifier {{ $member->first_name }}
                        {{ $member->last_name }}</h3>
                    <p class="text-[13px] text-slate-400">Édition des informations du choriste</p>
                </div>
            </div>
        </div>

        <form action="{{ route('admin.members.update', $member) }}" method="POST" enctype="multipart/form-data"
            class="space-y-6" x-data="{ photoPreview: null }">
            @csrf
            @method('PUT')
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input type="date" name="date_naissance"
                                    value="{{ old('date_naissance', $member->date_naissance ? \Carbon\Carbon::parse($member->date_naissance)->format('Y-m-d') : '') }}"
                                    class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg focus:border-[#7367F0] focus:ring-4 focus:ring-[#7367F0]/10 transition-all text-[#444050] text-[14px]">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <select name="pupitre_id" required
                                    ...
                                    @foreach($pupitres as $pupitre)
                                        <option value="{{ $pupitre->id }}" {{ old('pupitre_id', $member->pupitre_id) == $pupitre->id ? 'selected' : '' }}>{{ $pupitre->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="space-y-1.5">
                                ...
                                <select name="role_id" required
                                    ...
                                    @foreach($roles as $role)
                                        <option value="{{ $role->id }}" {{ old('role_id', $member->role_id) == $role->id ? 'selected' : '' }}>{{ $role->libelle }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="card-material p-6 sm:p-8 space-y-6">
                        ...
                        <div class="space-y-6">
                            <div class="space-y-1.5">
                                ...
                                <input type="text" name="citation" value="{{ old('citation', $member->citation) }}"
                                    class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg focus:border-[#7367F0] focus:ring-4 focus:ring-[#7367F0]/10 transition-all text-[#444050] text-[14px]"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div class="space-y-1.5">
                                    <label class="text-[12px] font-semibold text-slate-500 ml-1">Activité au sein de la
                                        chorale</label>
                                    <input type="text" name="activite" value="{{ old('activite', $member->activite) }}"
                                        class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg focus:border-[#7367F0] focus:ring-4 focus:ring-[#7367F0]/10 transition-all text-[#444050] text-[14px]"
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <input type="text" name="hobbie" value="{{ old('hobbie', $member->hobbie) }}"
                                        class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg focus:border-[#7367F0] focus:ring-4 focus:ring-[#7367F0]/10 transition-all text-[#444050] text-[14px]"
                            ...
                            <div class="space-y-1.5">
                                ...
                                <textarea name="love_choir" rows="3"
                                    ...
                                    placeholder="Partagez votre expérience...">{{ old('love_choir', $member->love_choir) }}</textarea>
                    ...
                    <div class="card-material p-6 sm:p-8">
                        ...
                        <div class="space-y-4">
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <div class="relative">
                                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $member->is_active) ? 'checked' : '' }}>
                                    <div
                                        class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#28C76F]">
                                    </div>
                                </div>
                                <span
                                    class="text-[14px] font-medium text-slate-600 group-hover:text-[#444050] transition-colors">Compte
                                    Actif</span>
                            </label>
                            <p class="text-[11px] text-slate-400">Désactiver le compte suspendra l'accès du membre à la
                                plateforme.</p>
                        </div>
                    </div>
                    ...
                    <div class="card-material p-6 sm:p-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="flex justify-center">
                                ...
                                <template x-if="!photoPreview">
                                    <div class="w-24 h-24 rounded-xl border-2 border-white shadow-material overflow-hidden">
                                        <img src="{{ $member->photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($member->first_name) . '&background=f8f7fa&color=7367f0&size=128' }}"
                            ...
                            <div @click="$refs.photoInput.click()"
                                ...
                                <span
                                    class="text-[11px] font-medium text-slate-500 group-hover:text-[#7367F0] transition-colors">Changer
                                    la photo</span>
