...
@section('title', 'Résultats Sondage - ' . $repetition->titre)

@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <a href="{{ route('admin.repetitions.index') }}" class="text-xs font-bold text-[#7367F0] uppercase tracking-widest hover:underline flex items-center gap-2 mb-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    Retour aux répétitions
                </a>
                <h1 class="text-2xl font-black text-[#444050] uppercase tracking-tight">{{ $repetition->titre }}</h1>
                <p class="text-sm text-slate-400 font-medium">Sondage de présence - {{ \Carbon\Carbon::parse($repetition->start_time)->translatedFormat('d F Y') }}</p>
            </div>
