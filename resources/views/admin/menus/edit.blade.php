...
@section('content')
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <div>
                <h1 class="text-2xl font-bold text-[#444050]">Modifier le Menu</h1>
                <p class="text-slate-500 text-sm">Modification de l'entrée : {{ $menu->name }}</p>
            </div>
        </div>

        <form action="{{ route('admin.menus.update', $menu->id) }}" method="POST"
            class="bg-white rounded-2xl shadow-material border border-slate-100 overflow-hidden">
            @csrf
            @method('PUT')
            <div class="p-8 space-y-6">
                <div class="space-y-1.5">
                    ...
                    <select name="module_id" required
                        ...
                        @foreach($modules as $module)
                            <option value="{{ $module->id }}" {{ old('module_id', $menu->module_id) == $module->id ? 'selected' : '' }}>{{ $module->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-1.5">
                    <label class="text-[12px] font-bold uppercase tracking-widest text-slate-500 ml-1">Icône (Nom Tabler
                        Icon)</label>
                    <input type="text" name="icone" value="{{ old('icone', $menu->icone) }}"
                        class="w-full px-4 py-3 rounded-lg border border-slate-200 outline-none focus:border-[#7367F0] focus:ring-4 focus:ring-[#7367F0]/10 transition-all font-semibold text-[#444050]"
                        placeholder="Ex: users, settings, home...">
                    @error('icone') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
