    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-slate-500 text-sm font-medium">Configurez les entrées de navigation de second niveau par
                    module.</p>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 border-b border-slate-100">
                            <th class="px-8 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest">Nom du Menu
                            </th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                            ...
                            <tr class="hover:bg-slate-50/50 transition duration-300">
                                ...
                                <td class="px-8 py-4">
                                    <div class="flex items-center justify-end gap-1">
                                        <button
                                            @click="openMenu({{ $menu->id }}, '{{ $menu->name }}', '{{ $menu->icone }}', '{{ $menu->module_id }}')"
                                            class="btn-icon">
                                        ...
                                        <form action="{{ route('admin.menus.destroy', $menu->id) }}" method="POST"
                                            onsubmit="return confirm('Attention: Supprimer ce menu supprimera tous ses sous-menus.');">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-8 py-12 text-center text-slate-400 italic font-medium">Aucun menu
                                    configuré.</td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="menuModal"
            ...
            <div class="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-material-lg"
                ...
                <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-slate-50/50">
                    <h3 class="font-bold text-lg text-[#444050]" x-text="editMode ? 'Modifier le Menu' : 'Nouveau Menu'">
                    </h3>
                ...
                <form :action="actionUrl" method="POST" class="p-8 space-y-6">
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[12px] font-bold uppercase tracking-widest text-slate-500 ml-1">Module
                            Parent</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[12px] font-bold uppercase tracking-widest text-slate-500 ml-1">Libellé du
                            Menu</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[12px] font-bold uppercase tracking-widest text-slate-500 ml-1">Icône (Nom Tabler
                            Icon)</label>
                        <input type="text" name="icone" x-model="formData.icone"
                            class="w-full px-4 py-3 rounded-lg border border-slate-200 outline-none focus:border-[#7367F0] focus:ring-4 focus:ring-[#7367F0]/10 transition-all font-semibold text-[#444050]"
                            placeholder="Ex: users, settings, home...">
                    </div>
